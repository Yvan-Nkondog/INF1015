...
// Constante ajoutée pour gérer les cas où la capacité
// est égale à zero (lorsqu'on essaie de changer
// la taille).
const size_t DEFAULT_MIN_ARRAY_SIZE = 1;
...
Concepteur* chercherConcepteur(std::string nomConcepteur, const ListeJeux& listeJeux)
{
	const Jeu* ptrJeu;
	Concepteur* ptrConcepteur;
	for (size_t i : iter::range(listeJeux.nElements)) {
		ptrJeu = listeJeux.elements[i];
		for (size_t j : iter::range(ptrJeu->concepteurs.nElements)) {
			ptrConcepteur = ptrJeu->concepteurs.elements[j];
			if (nomConcepteur == ptrConcepteur->nom) {
...
Concepteur* lireConcepteur(istream& fichier, const ListeJeux& listeJeux)
{
	...
	if (ptrConcepteur == nullptr) {
		...
		std::cout << "Allocation en mémoire du concepteur " << 
...
void ajouterJeu(ListeJeux& listeJeux, Jeu* jeu)
{
	if (listeJeux.nElements == listeJeux.capacite) {
		if (listeJeux.capacite > 0) {
			changerTaille(listeJeux, (2 * (listeJeux.capacite)));
		}
		else {
			changerTaille(listeJeux, DEFAULT_MIN_ARRAY_SIZE);
		}
	}
...
Jeu* lireJeu(istream& fichier, const ListeJeux& listeJeux)
{
	...
	//TODO: Enlever cet affichage temporaire servant à voir 
	// que le code fourni lit bien les jeux.
	for ([[maybe_unused]] size_t i : iter::range(jeu.concepteurs.nElements)) {
		...
		jeu.concepteurs.elements[i] = lireConcepteur(fichier, listeJeux);  
		//TODO: Ajouter le jeu à la liste des jeux auquel a participé le concepteur.
		ajouterJeu(jeu.concepteurs.elements[i]->jeuxConcus, &jeu);
	}
...
// Attention, ici il faut relâcher toutes les cases mémoires occupées par un jeu.
...
void afficherJeu(const Jeu& jeu) 
{
	...
	std::cout << "Annee de Sortie : " << jeu.anneeSortie << std::endl;
	std::cout << "Developpeur : " << jeu.developpeur << std::endl;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv)
{
	...
	//TODO: Faire les appels à toutes vos fonctions/méthodes pour voir 
	// qu'elles fonctionnent et avoir 0% de lignes non exécutées dans le 
	// programme (aucune ligne rouge dans la couverture de code; c'est 
	// normal que les lignes de "new" et "delete" soient jaunes).  Vous 
	// avez aussi le droit d'effacer les lignes du programmes qui ne sont 
	// pas exécutée, si finalement vous pensez qu'elle ne sont pas utiles.

	//TODO: Détruire tout avant de terminer le programme.  Devrait 
	// afficher "Aucune fuite detectee." a la sortie du programme; 
	// il affichera "Fuite detectee:" avec la liste des blocs, s'il 
	// manque des delete.
